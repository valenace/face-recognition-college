"""
asistencia_unificada_scrfd.py — Motor de Inferencia Biométrico en Tiempo Real con SCRFD
================================================================================
Arquitectura:
  - Tracking: ByteTrack para mantener IDs consistentes en multitudes.
  - Liveness: Gatekeeper activo con factor de escala.
  ...
  - Detección Facial: SCRFD para alta velocidad en CPU.
"""
...
class MotorAsistencia:
    def __init__(
        ...
        print("\n[INFO] Inicializando Motor de Asistencia con detector SCRFD...")
        ...
        self.detector = create_detector("scrfd")
    ...
    def iniciar_vigilancia(self, origen_video=0):
        ...
        cv2.namedWindow("Control de Asistencia (SCRFD)", cv2.WINDOW_NORMAL)
        ...
        try:
            while True:
                ...
                if len(detections_list) > 0:
                    ...
                    for tracked_item in tracked:
                        x1, y1, x2, y2 = map(int, tracked_item[0])
                        t_id = tracked_item[4]
                ...
                cv2.imshow("Control de Asistencia (SCRFD)", frame)
            ...
            print(f"\n{'=' * 50}")
            print(f" SESIÓN DE ASISTENCIA CERRADA (SCRFD)")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n❌ ERROR: {e}")
        print(
            "Asegúrate de tener creada la base de datos de embeddings ('data/database_embeddings.pkl') primero."
        )
