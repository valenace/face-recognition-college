    ...
    def enrolar_usuario(self, id_usuario, origen_video):
        ...
        try:
            while len(vectores_extraidos) < self.max_muestras:
                ...
                cv2.rectangle(display, (0, h_f - 20), (int(progreso * w_f), h_f), (0, 255, 0), -1)
                cv2.putText(display, f"{int(progreso*100)}%", (20, h_f - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                cv2.putText(display, mensaje_estado, (w_f // 2 - 100, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color_mensaje, 2)
    ...
    def enrolar_usuario_headless(self, id_usuario, ruta_video, sobrescribir=False):
        ...
        if len(vectores_extraidos) > 0:
            ...
            return {
                "exito": False,
                "muestras": 0,
                "mensaje": f"No se pudieron extraer embeddings del video ({frames_procesados} frames analizados).",
                "embedding": None
            }
