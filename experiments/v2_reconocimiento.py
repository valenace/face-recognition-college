...
print("\n[INFO] Cargando motores CV College (ONNX) y ByteTrack...")
...
# ============================================================
WINDOW_NAME = "Reconocimiento FAISS - CV College v2"
            ...
            for track in tracked_detections:
                ...
                if tracker_id in identidades_ancladas:
                    ...
                    if mejor_iou > 0.5 and rostro_asociado is not None:
                        ...
                        vec_actual_np = np.array([vec_actual.flatten()]).astype(
                            "float32"
                        )
                        ...
                        if max_similitud > UMBRAL_SIMILITUD:
                            ...
                            print(
                                f"  [ID:{tracker_id}] Voto {votos_actuales}/{VOTOS_REQUERIDOS} para {candidato} (Similitud: {max_similitud:.2f})"
                            )
                ...
                color_texto = (
                    (0, 0, 0)
                    if nombre_mostrar not in ["Desconocido", "Analizando..."]
                    else (255, 255, 255)
                )
                cv2.putText(
                    frame,
